...
fn test_create_and_metadata() {
    ...
    // CHD_CODEC_NONE = 0
    let mut chd = Chd::create(chd_path_str, 1024 * 1024, 4096, 512, [0, 0, 0, 0])
    ...
    chd.write_metadata(tag, 0, meta_data, 1)
...
fn test_hunk_io() {
    ...
    chd.read_hunk(0, &mut read_back)
        .expect("Failed to read hunk 0");
    ...
    chd.read_bytes(4096 + 50, &mut read_back_bytes)
